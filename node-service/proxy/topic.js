 ...
 */
exports.getTopics = function(topic_id, page, limit, callback){
	var limitInt = validator.toInt(limit);
	var pageInt = validator.toInt(page);

	if (validator.isNull(topic_id)) {
		Topic.find({},'',{sort: {topic_time : -1}, limit : limitInt, skip : pageInt}, callback);
	}
	else {
		Topic.find({topic_id: topic_id}, '', {sort: {topic_time : -1}, limit : limitInt}, callback);
	}
};
 ...
 * @param  {[type]}   page     获取分页
 * @param  {[type]}   limit    每页限制
 */
exports.getOwnTopics = function(user_id, page, limit, callback) {
	var limitInt = validator.toInt(limit);
	var pageInt = validator.toInt(page);
